...
The method \textbf{g:Dmarkarc(b,a,c,r,n,long,space,draw\_options)} marks the arc of a circle with center \emph{a}, radius \emph{r}, extending from \emph{b} to \emph{c}, with \emph{n} small segments. By default, the length (argument \emph{long}) is 0.25, and the spacing (argument \emph{space}) is 0.0625. The argument \emph{draw\_options} is a string (empty by default) that will be passed as is to the instruction \emph{\textbackslash draw}.

\subsubsection{Dmarkseg}
The method \textbf{g:Dmarkseg(a,b,n,long,space,angle,draw\_options)} marks the segment defined by \emph{a} and \emph{b} with \emph{n} small segments inclined at \emph{angle} degrees (45° by default). By default, the length (argument \emph{long}) is 0.25, and the spacing (argument \emph{space}) is 0.125. The argument \emph{draw\_options} is a string (empty by default) that will be passed as is to the instruction \emph{\textbackslash draw}.
...
\begin{itemize}
    \item The method \textbf{g:Dtangent\_from(A,p,t1,t2,dp,draw\_options,out)} draws the tangent(s) to the curve parameterized by \(p: t \mapsto p(t)\) (with complex values) originating from the point $A$ (a complex number). The arguments \emph{t1} and \emph{t2} represent the bounds of the search interval. The optional argument \emph{dp} is a function representing the derivative of the function $p$; by default, this argument is \emph{nil}, and the derivative of $p$ is replaced by an approximation. The optional argument \emph{draw\_options} is a string (empty by default) that will be passed as is to the \emph{\textbackslash draw} instruction. The optional argument \emph{out}, if used, must be the name of a variable. This variable must be an array, and at the end of execution, it will contain the points on the curve for which the tangent passes through $A$.

    \item The function \textbf{tangent\_from(A,p,t1,t2,dp)} returns the list of points on the curve parameterized by $p$, on the interval \emph{[t1;t2]}, for which the tangent passes through $A$ (a complex number). If there are no points, the function returns an empty list.
\end{itemize}
...
\begin{itemize}
    \item The method \textbf{g:Dparallelogram(a,u,v,draw\_options)} where \emph{a}, \emph{u}, \emph{v} represent three complex numbers denoting a vertex and two vectors, draws the parallelogram with vertices \emph{\{a,a+u,a+u+v,a+v\}}. The argument \emph{draw\_options} is a string (empty by default) which will be passed as is to the \emph{\textbackslash draw} instruction.
    \item The function \textbf{parallelogram(a,u,v)} returns the list of vertices of this parallelogram.
\end{itemize}
...
\begin{itemize}
    ...
    \item The \textbf{rectangle(a,b,c)} function returns the list of vertices of this rectangle.
\end{itemize}
...
\begin{itemize}
    ...
    \item \emph{dir=\{dirX,dirY,dep\}} is the writing direction. The three values ​​\emph{dirX}, \emph{dirY}, and \emph{dep} are three complex numbers representing three vectors. The first two indicate the writing direction, and the third indicates a translation of the label relative to the anchor point. The vector \emph{dep} is zero by default, and the vector \emph{dirY}, if absent, is equal to the vector \emph{dirX} rotated 90 degrees in the clockwise direction. By default, the \emph{dir} option is equal to the current value of the writing direction.
...
\begin{itemize}
    \item The \textbf{g:Labelstyle(position)} method allows you to specify the position of the labels relative to the anchor points. The \emph{position} argument is a string that can be: \emph{"N"} for north, \emph{"NE"} for northeast, \emph{"NW"} for northwest, or \emph{"S"}, \emph{"SE"}, \emph{"SW"}. By default, it is set to \emph{center}, in which case the label is centered on the anchor point.
    ...
    \item The \textbf{g:Labeldir(dir)} method allows you to manage the writing direction. The argument \emph{dir} is an table of the form \emph{dir=\{dirX, dirY, dep\}}. The three values ​​\emph{dirX}, \emph{dirY}, and \emph{dep} are three complex numbers representing three vectors. The first two indicate the writing direction, and the third indicates a translation of the label relative to the anchor point. The vector \emph{dep} is zero by default, and the vector \emph{dirY}, if absent, is equal to the vector \emph{dirX} rotated 90 degrees in the clockwise direction. When \emph{dir=\{\}}, this represents the usual writing direction.    
\end{itemize}
...
The path \emph{L} is composed of five pieces, which are:
    \begin{enumerate}
        \item \emph{\{Z(-3,2),"m"\}}: there is a data element and the instruction "m" which means \emph{moveto}. This instruction does not actually draw the path, but it allows a new component to begin, starting at the first point of the next piece. is $Z(-3,2)$ (the last point of the previous piece, if there is one, is not taken into account by this instruction; this is the only exception).

        \item \emph{\{Z(-3,2),-3,-2,"l"\}}: the first point of the second piece is indeed $Z(-3,2)$ and not $-3$, because $Z(-3,2)$ is the last point of the previous piece. Therefore, there are three data points, and the instruction "l" which means \emph{lineto}, is like executing the instruction \verb|g:Dpolyline( {Z(-3,2),-3,-2} )|, so these three points are connected by a piece. The last point of this piece is \emph{-2}.

        \item \emph{\{-2,0,2,2,-1,"ca"\}}: the first point of the third piece is indeed $-2$ and not $0$, because $-2$ is the last point of the previous piece. There are five data points, and the instruction "ca" stands for \emph{circle arc}. It's as if we were executing the instruction \verb|g:Darc(-2,0,2,2,-1)|, so the center is $0$, the arc goes from $-2$ to $2$ with a radius equal to $2$ and in a clockwise direction (last value $-1$). The last point of this piece is $2$.

        \item \emph{\{2,3,Z(3,3),0.5,"la"\}}: The first point of the fourth piece is $2$ (not $3$). There are four data points, and the instruction "la" stands for \emph{line arc}. This is a polygonal line with rounded corners and a circular arc of radius $0.5$ (the value preceding the instruction). The points of this line are \emph{2, 3, Z(3,3)}, so there will be a rounding to $3$. The last point of this piece is $Z(3,3)$.

        \item \emph{\{Z(3,3),1,Z(-1,5),Z(-3,2),"b"\}}: The first point of the fifth piece is $Z(3,3)$ (not $1$). The instruction "b" stands for \emph{Bezier}, so we draw a Bézier curve from $Z(3,3)$ to $Z(-3,2)$. The other two points, $1$ and $Z(-1,5)$, are the first and second control points of the curve.
\end{enumerate}
...
\noindent\textbf{Note}: In the example above, you can replace the part: \emph{Z(-3,2),"m", -3,-2,"l"}, with: \emph{Z(-3,2),-3,-2,"l"} because there is no other part before the \emph{moveto}.

\noindent\textbf{Available commands and their syntax}, the word \emph{last} denotes the last point of the previous piece:
\begin{itemize}
  \item \emph{z1,"m"} (moveto) starts a new component of the path at the point with affix $z1$.
  \item \emph{z1,...,zn,"l"} (lineto) draws the polyline \emph{\{last,z1,...,zn\}}.
  \item \emph{c1,c2,z2,"b"} (bézier) draws the Bézier curve \emph{\{last,c1,c2,z2\}}, where $c1$ and $c2$ are the two control points.
  \item \emph{z1,...,zn,"s"} (spline) draws the natural cubic spline through the points \emph{\{last,z1,...,zn\}}.
  \item \emph{z1,"c"} (circle) draws the circle with center $z1$ passing through the point \emph{last}. There is another possible syntax for the circle: \emph{z1,z2,"c"}, in which case the circle passing through the points \emph{last}, $z1$, and $z2$ is drawn.
  \item \emph{z1,z2,r,sens,"ca"} (circular arc) draws a circular arc with center $z1$ and radius $r$, going from \emph{last} to $z2$, counterclockwise when \emph{sens=1} (and therefore clockwise when \emph{sens=-1}).
  \item \emph{z1,z2,rx,ry,sens,inclinaison,"ea"} (elliptic arc) draws an elliptic arc with center $z1$ going from \emph{last} to $z2$; $rx$ and $ry$ are the two radii along the two axes of the ellipse; \emph{inclinaison} is the angle in degrees between the first axis of the ellipse (the one corresponding to $rx$) and the horizontal. The parameter \emph{inclinaison} is optional and defaults to $0$.
  \item \emph{z1,rx,ry,inclinaison,"e"} (ellipse) draws the ellipse with center $z1$ passing through \emph{last}; $rx$ and $ry$ are the two radii along the two axes of the ellipse; \emph{inclinaison} is the angle in degrees between the first axis of the ellipse (the one corresponding to $rx$) and the horizontal. The parameter \emph{inclinaison} is optional and defaults to $0$. When the point \emph{last} is not on this ellipse, a segment is drawn between this point and a point on the ellipse.
  \item \emph{z1,...,zn,r,"la"} (line arc) draws the polyline \emph{\{last,z1,...,zn\}} while replacing each “corner” by a circular arc of radius $r$.
  \item \emph{z1,...,zn,r,"cla"} (closed line arc) same as the previous command, except that the polyline is closed.
  \item \emph{"cl"} (closepath) this command is used alone; it closes the current component by drawing a segment joining the last point to the first point of the current component.
\end{itemize}
...
\begin{itemize}
    \item The function \textbf{path(path,nbdots)} returns a polygonal line containing the points that make up the \emph{path}. The optional argument, \emph{nbdots}, is the minimum number of points calculated for each Bézier curve; its default value is the global variable \emph{bezier\_nbdots}, which is initialized to $8$.
...
The formula that defines the label at tick mark number $n$ is: \textbf{(originnum + unit*n)"labeltext"/labelden}.
    ...
    \item \opt{legendstyle=\{"auto","auto"\}}. Specifies the label style for the legends; with the value \emph{"auto"}, it is determined automatically; otherwise, the following values ​​can be used: "N", "NW", "W", "SW", "S", "SE", "E", "NE".
...
The formula that defines the label at tick mark number $n$ is: \textbf{(originnum + unit*n)"labeltext"/labelden}.
    ...
    \item \opt{legendstyle="auto"}. Specifies the label style for the legend. With the value \emph{"auto"}, the style is determined automatically; otherwise, the following values ​​can be used: "N", "NW", "W", "SW", "S", "SE", "E", "NE".
...
The formula that defines the label at graduation number $n$ (at point $A+nu$) is: \textbf{(originnum + unit*n)"labeltext"/labelden}.
    ...
    \item \opt{legendstyle="auto"}. Specifies the label style for the legend. With the value \emph{"auto"}, the style is determined automatically; otherwise, the following values ​​can be used: "N", "NW", "W", "SW", "S", "SE", "E", "NE".
    \item \opt{labelpos="bottom"}. This option specifies the position of the labels relative to the axis. The possible values ​​are: "none", "bottom", or "top". This position also determines the position of the legend: on the opposite side of the axis.
    \item \opt{labelden=1}. This option specifies the denominator of the labels (integer) for the axis. The formula that defines the label at graduation number $n$ is: \textbf{(originnum + unit*n)"labeltext"/labelden}.
...
To import an image into the current graphic (and possibly draw on top of it), use the method:\par
\hfil\textbf{g:Dimage( filename, anchor, options )},\hfil\par
where:
\begin{itemize}
    \item \emph{filename} is the full name of the image file. The image will be displayed using the \verb|\includegraphics[]{}| command within a node.
    \item \emph{anchor} is a complex number representing the anchor point of the image in the graphic.
    \item \emph{options} is a table whose fields define the display parameters (with their default values):
    \begin{itemize}
        \item \opt{pos = "center"} specifies the position of the image relative to the anchor point, following the same principle as for labels. Possible values are: "center", "N", "NE", "E", "SE", "S", "SW", "W", and "NW".
...
This can be done using the method: \textbf{g:Dmapimage( filename, parallelo, options )}, where:
\begin{itemize}
    \item \emph{filename} is the full name of the image file.
    \item \emph{parallelo} is a list of the form \emph{\{a, u, v\}}, consisting of three complex numbers. This list defines the parallelogram with vertices \{a, a+u, a+u+v, a+v\}.
    \item \emph{options} is a table whose fields specify the parameters (with their default values):
    \begin{itemize}
        ...
        \item \opt{border\_options = nil}, when set to a value other than \emph{nil}, must be a string containing the drawing options for the boundary of the parallelogram. The value of \emph{border\_options} is passed directly to the \verb|\draw| command to render the outline.
    \end{itemize}
\end{itemize}
...
The (optional) argument \emph{table} is a boolean (false by default) that indicates whether the result should be returned as a table \verb|{r,g,b}| or not (by default it is as a string).
    ...
    \item The method \textbf{g:Newcolor(name,color)} allows you to define a new color in the tikz export to rgb format. The name of this color will be \emph{name} (string). The argument \emph{color} can be either a table of three components: red, green, blue (between 0 and 1) defining this color, or a string representing a color. In the first case, the method uses a \verb|\definecolor|, and in the second case, it uses a \verb|\colorlet|.
