...
The function \textbf{polyline2path(L)} where \emph{L} is a list of complex numbers or a list of lists of complex numbers, returns \emph{L} as a path (which can be drawn with the \emph{g:Dpath()} method).
...
The function \textbf{read\_csv\_file(file, options)} reads a \emph{csv} file. The argument \emph{file} is a string representing the file with the extension: \emph{"<name>.csv"}. The argument \emph{options} is an table whose fields represent the function's parameters, which are (with their default values):
\begin{itemize}
    \item \opt{header = true}, with the value \emph{true} this means that the first line of the file contains the column names.

    \item \opt{dic = false}, with the value \emph{true} this means that the returned result will be a list of dictionaries (one per line), the keys of these dictionaries being the column names. With the value \emph{false} (default), the result is a list of lists of values ​​(one list of values ​​per line). When the \emph{header} option is set to \emph{false}, the \emph{dic} option automatically remains \emph{false}.
...
\begin{enumerate}
    ...
    \item The list of values ​​in the first line when the \emph{header} option is set to \emph{true}.
\end{enumerate}
