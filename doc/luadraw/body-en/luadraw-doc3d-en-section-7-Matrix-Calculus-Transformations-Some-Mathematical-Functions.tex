...
\begin{itemize}
\item The function \textbf{pxy(L,z0)} returns the image of $L$ by the orthogonal projection onto the $z=z0$ plane (by default $z0=0$).
\item The function \textbf{pyz(L,x0)} returns the image of $L$ by the orthogonal projection onto the $x=x0$ plane (by default $x0=0$).
\item The function \textbf{pxz(L,y0)} returns the image of $L$ by the orthogonal projection onto the $y=y0$ plane (by default $y0=0$).
...
The function \textbf{interCS(C,S)} calculates and returns (if it exists) the intersection between the circle $C=\{A,r,n\}$ ($A$ is the center of the circle, $r$ the radius, and $n$ a normal vector to the plane of the circle), and the sphere $S=\{B,R\}$ ($B$ is the center of the sphere and $R$ the radius). The function returns either \emph{nil} (empty intersection), a single point, or two points (sequence).
