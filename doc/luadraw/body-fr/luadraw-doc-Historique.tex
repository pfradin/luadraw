...
\begin{itemize}
    ...
    \item La méthode \emph{g:Dshadedpolyline()} fait désormais partie de l'extension \emph{luadraw\_shadedforms}.    
    \item Ajout des méthodes \emph{g:Dimage()} et \emph{g:Dmapimage()}, la première permet l'inclusion d'une image dans le graphique, la deuxième permet de mapper une image sur un parallélogramme.
    \item Ajout de la fonction \emph{parallelogram()} qui renvoie les sommets d'un parallélogramme construit à partir d'un sommet et de deux vecteurs. Ajout également de la méthode de dessin \emph{g:Dparallelogram()} correspondante.
    \item Ajout des options \emph{gradside} et \emph{gradsection} qui permettent de modifier les paramètres du gradient dans le dessin des cylindres, des cônes et des cônes tronqués (méthodes \emph{g:Dcylinder()}, \emph{g:Dcone()} et \emph{g:Dfrustum()}.
    \item Extension de la méthode \emph{g:Newcolor(name,color)}, le second argument peut maintenant être soit une table de trois composantes RGB, soit une chaîne de caractères représentant une couleur.
