...
\begin{itemize}
    ...
    \item The product by a scalar, if $k$ is a real number, \code{k*M(x,y,z)} returns \val{M(k*x,k*y,k*z)}.
...
\begin{itemize}
    ...
    \item \cmd{pt3d.isobar3d(L)}: Returns the isobarycenter of the 3D points in the list (table) \argu{L} (elements of \argu{L} that are not 3D points are ignored).

    \item \cmd{pt3d.insert3d(L, A \fac{, epsilon})}: This function inserts the 3D point \argu{A} into the list \argu{L}, which must be a \textbf{variable} (and which will therefore be modified). Point \argu{A} is inserted \textbf{without duplicates} and the function returns its position (index) in list \argu{L} after insertion. The (optional) argument \argu{epsilon} is $0$ by default, indicating how closely the comparisons are made.
\end{itemize}
