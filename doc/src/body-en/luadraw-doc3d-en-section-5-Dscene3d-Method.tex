...
In both cases, the \argu{options} argument is a table whose fields define these options (with their default values) are:
...
The method \cmd{g:addPolyline(L, options)}, where \argu{L} is a list of 3D points, or a list of lists of 3D points, adds \argu{L} to the scene. The \argu{options} argument is a table whose fields define these options (with their default values) are:
\begin{itemize}
    \item \opt{style=<current line style>}: to set the line style; this is the current style by default.
    \item \opt{color=<current line color>}: line color.
    ...
    \item \opt{width=<current line width>}: line thickness in tenths of a point.
...
The \cmd{g:addDots(dots, options)} method allows you to add 3D points to the scene. The argument \argu{dots} is either a 3D point or a list of 3D points. The optional argument \argu{options} is a table whose fields define the options, these options are (with the default value):
\begin{itemize}
...
g:Dpath3d( F,"fill=Beige,fill opacity=0.6") -- drawing of the perforated plane
