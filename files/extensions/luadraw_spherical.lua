...
local insidelabelcolor = "darkgray"
...
function graph3d:Define_sphere( args )
    ...
    insidelabelcolor = args.insidelabelcolor or "DarkGray"
    arrowBstyle = args.arrowBstyle or "->"
    arrowAstyle = args.arrowBstyle or "<-"
...
-- options = {style=, color=, width=, opacity=, arrows=}
    ...
    local add_seg_out = function(U,V,arrow)
        ...
        if #dev ~= 0 then 
            ...
            if hidden and hiddendelayed and (style ~= "noline") then
                table.insert(hidden_part, {dev,ld.Hiddenlinestyle,color,width,opacity,arrow}) 
            end
        end
    end
